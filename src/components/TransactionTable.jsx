...
export default function TransactionTable({ 
  ...
  return (
    <div className="dash-card" style={{ background: '#fff', borderRadius: 16, border: '1px solid #e2e8f0', boxShadow: '0 4px 6px -1px rgba(0,0,0,0.02)', overflow: 'hidden' }}>
      <div style={{ padding: '20px 24px', borderBottom: '1px solid #e2e8f0', display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '16px' }}>
        ...
        <button className="export-btn-hover" onClick={handleExportCSV} style={{ background: '#f8fafc', border: '1px solid #e2e8f0', padding: '6px 12px', borderRadius: 6, fontSize: 12, fontWeight: 600, cursor: 'pointer' }}>Export CSV</button>
      </div>
      ...
      <style>{`
        .tx-row-hover { transition: background 0.2s ease, transform 0.2s ease; cursor: default; }
        .tx-row-hover:hover { background: #f8fafc; transform: translateX(4px); }
        .export-btn-hover { transition: all 0.2s ease; }
        .export-btn-hover:hover { background: #eef2ff !important; border-color: #4f46e5 !important; color: #4f46e5; }
      `}</style>
    </div>
  );
}